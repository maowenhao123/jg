...
-(void)countFrom:(float)startValue to:(float)endValue attDict:(NSDictionary *)attDict withPrefix:(NSString *)prefix withDuration:(NSTimeInterval)duration
{
    ...
    [self countFrom:startValue to:endValue withDuration:duration];
}
...
-(void)updateValue:(NSTimer*)timer
{
    ...
    else
    {
        ...
        {
            text = [NSString stringWithFormat:self.format,(int)value];
        }
        else
        {
            text = [NSString stringWithFormat:self.format,value];
        }
        NSMutableAttributedString *attStr = [[NSMutableAttributedString alloc] initWithString:text];
        [attStr appendAttributedString:[[NSAttributedString alloc] initWithString:@"%"]];
        if(_prefix)
        {
            [attStr insertAttributedString:[[NSAttributedString alloc] initWithString:_prefix] atIndex:0];
        }
        if(_attDict)
        {
            [attStr addAttributes:_attDict range:NSMakeRange(0, attStr.length - 1)];
        }
